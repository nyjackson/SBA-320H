...
function Information({ p, dispatch, info }) {
  ...
  async function getInformation() {
    try {
      ...
      const apiUrl = isCat
        ? `https://api.thecatapi.com/v1/images/${p.id}`
        : `https://api.thedogapi.com/v1/images/${p.id}`;
  ...
  return (
    <div className="info">
      ...
      <p>
        Temperament: {info?.temperament || "N/A"} <br></br>Life Span:{" "}
        {info?.life_span || "N/A"}
        <br></br>Origin: {info?.origin || "N/A"}
      </p>
    </div>
  );
}
